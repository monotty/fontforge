...
static int dirmatch(char* dirname, char* pattern, char* buffer, size_t buffer_size)
{
	DIR* dir;
	struct dirent* ent;
	...
	while ((ent = readdir(dir)) != NULL)
	{
		...
		{
			strncpy(buffer, dirname, buffer_size);
			strncat(buffer, "/", buffer_size);
			strncat(buffer, ent->d_name, buffer_size);
			closedir(dir);
			return(1);		/* Good */
		}
	}
...
static int dirfind(char* dir, char* pattern, char* buffer, size_t buffer_size)
{
	char* pt, * space;
	int ret = dirmatch(dir, pattern, buffer, buffer_size);
	...
	if (ret == -1)
	{
		...
		{
			*pt = '\0';
			ret = dirmatch(space, pattern, buffer, buffer_size);
		}
		free(space);
	}
	if (ret == -1)
	...
	if (!ret)
	{
		...
		strncat(buffer, "/", buffer_size);
		strncat(buffer, pattern, buffer_size);
	}
	return(ret);
}
...
{
	int fliename_size = strlen(dir) + strlen("/Intmetrics") + 3;
	char* filename = malloc(fliename_size);
	...
	if (dirfind(dir, "IntMet?", filename, fliename_size))
		file = fopen(filename, "rb");
	else if (dirfind(dir, "Intmetric?", filename, fliename_size))
		file = fopen(filename, "rb");
	if (file == NULL)
	{
...
static void FindEncoding(SplineFont* sf, char* filename)
{
	...
	strncpy(pattern, "Base *", sizeof(pattern));
	pattern[4] = filename[strlen(filename) - 1];
	pt = strrchr(filename, '/');
	if (pt != NULL)
		*pt = '\0';

	int otherdir_size = strlen(filename) + strlen("/../Encodings") + 5;
	otherdir = malloc(otherdir_size);
	strncpy(otherdir, filename, otherdir_size);
	strncat(otherdir, "/../Encodings", otherdir_size);

	int encfilename_size = strlen(otherdir) + strlen("base0encoding") + 20;
	encfilename = malloc(encfilename_size);

	if (dirfind(otherdir, pattern, encfilename, encfilename_size))
		file = fopen(encfilename, "rb");
	else if (dirfind(filename, pattern, encfilename, encfilename_size))
		file = fopen(encfilename, "rb");
	free(otherdir);
...
{
	int filename_size = strlen(dir) + strlen("/Outlines*") + 3;
	...
	if (dirfind(dir, "Outlines*", filename, filename_size))
		file = fopen(filename, "rb");
	if (file == NULL)
	{
		fprintf(stderr, "Couldn't open %s\n", filename);
